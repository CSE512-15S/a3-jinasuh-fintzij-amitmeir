viz.legend = function (containerId) {
    ...
    legendInstance.update = function () {
        if (data && !initialized) {
            initialized = true;
            var logFoiRange = [0, 1, 2, 3, 4, 5, 6, 7];
            var withZero = data.colorScheme.slice();
            withZero.unshift("#eee");
            var foiThreshold = d3.scale.threshold().domain(logFoiRange).range(withZero);
            var legend = d3.svg.legend().units("log(1+FOI)").cellWidth(80).cellHeight(25).inputScale(foiThreshold).cellStepping(100);
